...
interface ISearchProps {
    ...
    fields: ISearchField[];
    handleSearch: (args: ISearchData) => void;
}
 ...
 * @returns 
 */
export const DefaultSearch = ( { initialData, fields, handleSearch }: ISearchProps ) => {
        ...
        <div className="border border-gray-250 rounded-lg p-6">
            <dl className="grid grid-cols-1 sm:grid-cols-6 gap-x-4">
                ...
                <div key={ field.value as string } className="px-4 sm:col-span-3 sm:px-0">
                    ...
                    <dd className="mt-1 text-sm leading-6 text-muted-foreground mt-2">
                        {/** 텍스트 필드 일 경우 */
                        field.type === 'text' && (
                            <Input id={ field.value as string } className="mb-4" value={ filters[ field.value as string ] } onChange={ ( e ) => setFilters( { ...filters, [field.value as string]: e.target.value } ) } />
                        ) }
            ...
            <div className="grid grid-cols-2 pt-4">
                <div className="w-full flex items-center gap-2">
                    <Button variant="outline" className="cursor-pointer" onClick={ () => setFilters( initialData ) }>초기화</Button>
                </div>
                <div className="w-full flex items-center justify-end gap-2">
                    <Button variant="outline" className="cursor-pointer">더보기</Button>
