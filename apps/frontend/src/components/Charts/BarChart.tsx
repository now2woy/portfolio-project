            ...
            <CardContent>
                <ChartContainer config={ chartConfig }>
                    <BarChart accessibilityLayer data={ chartData }>
                        ...
                        <XAxis dataKey={ xDataKey } tickLine={ false } tickMargin={ 10 } axisLine={ false } tickFormatter={ ( value ) => value.slice( 5, 10 ) } />
                        <ChartTooltip cursor={ false } content={ <ChartTooltipContent hideLabel /> } />
                        { chartSeries.map( ( series, index ) => (
                            <Bar key={ index } dataKey={ series.key } fill={ series.fillColor } radius={ 8 } />
                        ) ) }
                    </BarChart>
                </ChartContainer>
            </CardContent>
            ...
            <CardContent>
                <ChartContainer config={ chartConfig }>
                    <BarChart accessibilityLayer data={ chartData } layout="vertical" margin={ { left: -20 } } >
                        ...
                        <YAxis dataKey={ yDataKey } type="category" tickLine={ false } tickMargin={ 10 } axisLine={ false } tickFormatter={ ( value ) => value.slice( 5, 10 ) } />
                        <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel />} />
                        { chartSeries.map( ( series, index ) => (
                            <Bar key={ index } dataKey={ series.key } fill={ series.fillColor } radius={ 5 } />
                        ) ) }
                    </BarChart>
                </ChartContainer>
            </CardContent>
